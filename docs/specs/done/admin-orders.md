...
нормативных разделах: поиск CDEK выбирает ПВЗ, сервер повторно проверяет код
...
`GET /api/checkout/delivery` не требует admin-сессии. При
`DELIVERY_ENABLED=false` он возвращает `{ enabled:false }`; при включённой
доставке — `{ enabled:true, cdekPickupDeliveryKopecks }` с `Cache-Control:
no-store`, а при отсутствии тарифа — `503 DELIVERY_UNAVAILABLE`.
...
4. При включённой доставке покупатель вводит город, получает через `/api/cdek`
   список ПВЗ и выбирает точку. В форме остаётся читаемая карточка
   `город · название · адрес`; ручной ввод/редактирование кода и отправка без
   выбранной точки невозможны. При `DELIVERY_ENABLED=false` этот блок не
   показывается.
...
При включённой доставке до загрузки тарифа и выбора ПВЗ кнопка оплаты disabled. Ошибка выбора
...
Интеграция CDEK API — не «просто поле»: доступность ПВЗ изменяется, а код
нельзя доверять браузеру. Нужны мобильная проверка поиска и server-side
подтверждение ПВЗ. Это внешняя интеграция, поэтому по `PROJECT_CORE.md` §2
Пауза 2 требуется перед её включением на production.
