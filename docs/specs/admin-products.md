...
Статус: ✅ готова к реализации. Пауза 2 на `iron-session` пройдена — владелец
подтвердил зависимость 2026-06-20 (§13).
...
гарантирует минимум. При DELETE текущей обложки следующая по sort-order становится
...
- **`app/admin/page.tsx` — список товаров.** Таблица/карточки: обложка, название,
  ...
  `public` и отправляет полный список ID в `reorder`; при `409` экран заново
  получает список и сообщает об изменении другим действием.
