...
**Версия:** 1.2 · 2026-06-25  
**Статус:** ГОТОВА К РЕАЛИЗАЦИИ (после ревью v1.2)
...
вручную в ЛК СДЭК (как в `cdek-manual-launch.md`). Включается после теста на sandbox.
...
## Накладная (waybill PDF)

Накладная генерируется асинхронно на стороне СДЭК.
...
2. drainCdekOutbox() для poll_waybill:
   ...
   b. POST /v2/print/orders { "orders": [{ "order_uuid": uuid }], "type": "waybill" }
      Получить print_task_uuid.
   c. GET /v2/print/orders/{print_task_uuid} — ждать status == "SUCCESSFUL"
      Если нет — available_at += 5 с, retry.
   d. Сохранить url → orders.cdek_waybill_url.
```

URL накладной — прямая ссылка на PDF на серверах СДЭК. В админке отображать
кнопкой «Скачать накладную».
...
- **Бюджеты попыток РАЗДЕЛЕНЫ по task_type** (один `attempt_count`, разная политика):
  ...
  - `poll_waybill`: ожидание `state=SUCCESSFUL` (CDEK генерит накладную долго) —
...
И загрузить sandbox-ключи в настройки. На проде `CDEK_API_BASE` не задаётся (дефолт).
...
Все вопросы закрыты. Перед запуском на проде:

- Создать тестовый ключ в ЛК СДЭК → «Интеграция» (отдельная пара client_id/secret для sandbox)
...
| Тест на sandbox | да, перед выходом в бой (отдельный тестовый ключ в ЛК СДЭК) |
