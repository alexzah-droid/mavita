...
| FR-CAT-3 | Галерея фото товара | Несколько фото, обложка (`is_cover`) — главная на витрине; на мобильных и планшетах крупные фото листаются горизонтальным свайпом | `app/components/ProductGallery.tsx`, `product_images` |
...
| FR-CAT-6 | Атрибуты товара | Серия, подзаголовок, описание, ароматы (`scent[]`); физические характеристики — вес изделия, вес чистого воска, время горения, состав воска, фитиль (опциональны, пустые скрываются; миграции `015`/`020`/`022`) | `products`, `app/product/[slug]/page.tsx` |
