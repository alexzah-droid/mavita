...
// I1: подпись считается на сервере как MD5(login:OutSum:InvId:Password1).
describe('buildPaymentUrl (I1)', () => {
  it('кладёт в URL OutSum, InvId и корректную подпись на Password1', () => {
    const url = new URL(buildPaymentUrl(5, 180000, 'a@b.ru', 'Заказ №5'))
    ...
    expect(url.searchParams.get('SignatureValue')).toBe(md5(`${LOGIN}:1800.00:5:${PW1}`))
  })

  it('подпись детерминирована для одних и тех же входных данных', () => {
    const a = new URL(buildPaymentUrl(7, 90000)).searchParams.get('SignatureValue')
    const b = new URL(buildPaymentUrl(7, 90000)).searchParams.get('SignatureValue')
    expect(a).toBe(b)
  })

  it('добавляет IsTest=1 только в тестовом режиме', () => {
    expect(new URL(buildPaymentUrl(1, 1000)).searchParams.has('IsTest')).toBe(false)
    process.env.ROBOKASSA_TEST_MODE = 'true'
    expect(new URL(buildPaymentUrl(1, 1000)).searchParams.get('IsTest')).toBe('1')
  })
})
