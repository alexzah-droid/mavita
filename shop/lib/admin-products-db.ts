...
type Row = {
  ...
  burn_time_hours: number | null; wax: string | null; wick: string | null
}
const SELECT = `SELECT p.id, p.slug, p.name, p.series, p.subtitle, p.description, p.price_kopecks, p.scent, p.in_stock,
  ...
  p.weight_grams, p.box_length_cm, p.box_width_cm, p.box_height_cm, p.burn_time_hours, p.wax, p.wick,
...
export function adminProduct(row: Row, now = new Date()): AdminProduct {
  ...
  return { id: row.id, slug: row.slug, name: row.name, series: row.series, subtitle: row.subtitle, description: row.description,
    ...
    burnTimeHours: row.burn_time_hours ?? null, wax: row.wax ?? null, wick: row.wick ?? null }
}
...
function fields(input: ValidatedProductInput, create: boolean) {
  ...
  const parts: string[] = []
  for (const [key, column] of Object.entries({ name: 'name', slug: 'slug', series: 'series', subtitle: 'subtitle', description: 'description', priceKopecks: 'price_kopecks', scent: 'scent', inStock: 'in_stock', visibility: 'visibility', weightGrams: 'weight_grams', boxLengthCm: 'box_length_cm', boxWidthCm: 'box_width_cm', boxHeightCm: 'box_height_cm', burnTimeHours: 'burn_time_hours', wax: 'wax', wick: 'wick' })) {
