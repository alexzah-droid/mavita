...
function request(password: string, ip = 'test-ip') { return new Request('http://localhost/api/auth/login', { method: 'POST', headers: { origin: 'http://localhost', host: 'localhost', 'content-type': 'application/json', 'x-forwarded-for': ip }, body: JSON.stringify({ password }) }) }
describe('POST /api/auth/login', () => {
  ...
  it('requires same origin before parsing credentials', async () => {
    const { POST } = await import('@/app/api/auth/login/route'); const response = await POST(new Request('http://localhost/api/auth/login', { method: 'POST', headers: { origin: 'https://evil.example', host: 'localhost', 'content-type': 'application/json' }, body: '{bad json' }))
    expect(response.status).toBe(403)
  })
  it('logout is idempotent and clears a session cookie', async () => {
    const { POST } = await import('@/app/api/auth/logout/route'); const response = await POST(new Request('http://localhost/api/auth/logout', { method: 'POST', headers: { origin: 'http://localhost', host: 'localhost' } }))
