...
| Ф2 | Корзина и оформление | 🚧 | Корзина (готова); оформление заказа (далее) |
...
**Тесты:** `lib/cart.test.ts` — добавление, слияние дублей, удаление, изменение количества,
подсчёт суммы в копейках.

**Далее (оформление):**
- ⬜ `app/checkout/page.tsx` — форма: имя, email, телефон.
- ⬜ `app/api/orders/route.ts` — создание заказа (`pending`) + `order_items` (snapshot цены).
- ⬜ `app/order/[id]/page.tsx` — «заказ принят».
