...
11. **✅ Подключить базовую аналитику.**
   - Яндекс.Метрика `110274888` развернута на production: публичные маршруты,
     SPA-hit для App Router, `dataLayer` под будущий ecommerce.
   - Не трекаем `/admin*` и `/order*`, чтобы не отправлять служебные и
     персональные URL в аналитику.
   - Документировано в `docs/specs/done/yandex-metrika.md`; `/privacy`
     обновлена под веб-аналитику.
   - Следующие marketing/growth-задачи остаются отдельными: цели в кабинете
     Метрики, ecommerce events, отзывы, при необходимости GA4.
